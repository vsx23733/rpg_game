...
from src.dialog import Conversation, DialogBox
...
class MapManager:
    ...
    def __init__(self, screen, player):
        ...
        self.register_map("world", portals=[
            ...
            NPC("paul", nb_points=4, npc_texts=["Fake Aivancity", "comme l'enculé a dit "], player_choice={'A':
                "Je suis sur que l'enculé c'est Florian",
                'B': "Je suis su que l'enculé c'est Ephraïm", 'C':"Je suis sur que l'enculé c'est Axel"})
        ])
        ...
        self.register_map("dungeon", portals=[
            ...
            NPC("boss", nb_points=2, npc_texts=["Petit, va dormir", "Tu ne dors pas ? Imbecile"], player_choice={'A':
                "Je ne dort pas tu vas faire quoi",
                'B': "Je dors si je veux", 'C':"Je dors si je veux"})
        ])
    ...
    def check_npc_collisions(self, dialog_box):
        for sprite in self.get_group().sprites():
            if sprite.feet.colliderect(self.player.rect) and type(sprite) is NPC:
                conversation = Conversation(npc=sprite, player_choices=sprite.player_choice)
                dialog_box.execute(conversation=conversation)
